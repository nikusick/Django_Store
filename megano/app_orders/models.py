...
class Order(models.Model):
    ...
    def get_total_cost(self):
        ...
        if self.deliveryType == "express":
            result += OrderPriceConstants.objects.get(
                title="EXPRESS_DELIVERY_PRICE"
            ).value
        elif (
            result
            < OrderPriceConstants.objects.get(title="MIN_PRICE_FOR_FREE_DELIVERY").value
        ):
...
class Payment(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, default=None, related_name="payment"
    )
